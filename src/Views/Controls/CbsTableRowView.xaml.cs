...
using static CbsContractsDesktopClient.Services.Table.TableCellPresentationBuilder;
using System;
...
namespace CbsContractsDesktopClient.Views.Controls
{
    public sealed partial class CbsTableRowView : UserControl
    {
        ...
        internal static string GetCellText(CbsTableColumnDefinition column, TableDataRow row, bool showStageCostFraction)
            => TableCellPresentationBuilder.GetCellText(column, row, showStageCostFraction);
        ...
        private void UpdateCellContent()
        {
            ...
            var presentation = TablePresentationResources.Capture(ShowStageCostFraction, Density);
            ...
            for (var index = 0; index < Columns.Count; index++)
            {
                ...
                {
                    var cell = TableCellPresentationBuilder.Build(Columns[index], Row!, RowStyleKey, presentation);
                    if (IsBadgeTemplate(Columns[index]))
                    {
                        _badgeTexts[index].Text = cell.Text;
                        _badgeTexts[index].Foreground = new SolidColorBrush(TablePresentationResources.ToColor(cell.Foreground));
                        _badgeCells[index].Background = new SolidColorBrush(TablePresentationResources.ToColor(cell.Background));
                        _textCells[index].Text = string.Empty;
                    ...
                    {
                        _textCells[index].Text = cell.Text;
                        _textCells[index].Foreground = new SolidColorBrush(TablePresentationResources.ToColor(cell.Foreground));
                        _textCells[index].FontWeight = cell.Bold ? FontWeights.SemiBold : FontWeights.Normal;
                    }
                }
            }
        }
        ...
        {
            if (isPlaceholder || Row is null || IsSelected || IsHovered || IsPressed)
                return;
            var style = TableCellPresentationBuilder.GetRowStyle(Row, RowStyleKey);
            if (style.BackgroundBrushKey is not null)
