...
2. Позиция содержит материал/СЗИ, количество и состояние поставки `Потребность` либо `В наличии`.
...
- будущая таблица `Потребность` работает с непривязанными к заказу `StageOrder`;
...
- важность коммерческой позиции ограничена значениями `0 — Потребность` и `1 — В наличии`;
...
5. Создать отдельный экран `Потребность`.
