...
2. Подготовить shell/content слой к страницам с несколькими таблицами одновременно, в первую очередь к отчету `Активность`
3. Отполировать contract-oriented `DetailView` для сложных таблиц
4. Начать разработку отчета `Активность`
5. Развивать `Pauli.WinUiKit` как библиотеку компактных reusable WinUI controls
6. Расширять CRUD-слой справочников от базового create/update/delete к details и доменным ограничениям
7. При росте числа экранов выделить более формальный `INavigationService` и реестр страниц
8. Поддерживать audit panel как общий cross-reference сценарий для всех новых таблиц

## Ближайшая следующая задача

Следующий production-шаг: использовать завершенную декомпозицию content host при разработке отчета `Активность`.
...
- следующий крупный экран `Активность` потенциально будет использовать несколько таблиц одновременно и должен опираться на уже выделенные владельцы поведения
...
5. Отмечать повторяемые controls, которые стоит вынести или доработать в `Pauli.WinUiKit`.
6. Сохранить существующее поведение `/revisions` и `/stages` через регрессионные тесты.
7. Проверить, что новый pattern подходит для отчета `Активность` с несколькими таблицами.

После этого:

- шлифовать `DetailView` для сложных таблиц
- разрабатывать отчет `Активность` поверх более чистого content/table pattern
- развивать `Pauli.WinUiKit` без зависимости от CBS-domain моделей
